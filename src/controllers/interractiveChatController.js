...
module.exports = {
    async execute(client, message, prompt) {
        const newPrompt = `Respond to this prompt:\n${prompt} following these instructions:\n
        ...
        const data = JSON.parse(await gpt4Controller.runPrompt(newPrompt));
        ...
        switch (data.type) {
            case 'TEXT': {
                const textPrompt = `${message.author} sent you this message: ${data.content}, respond to it following these instructions:\n
                ...
                message.channel.send(
                    await gpt4Controller.runPrompt(textPrompt)
                );
                break;
            }
            case 'PICTURE':
                message.channel.sendTyping();
                message.channel.send(
                    await dallEController.runPromptWithArgs(data.content)
                );
                break;
            case 'VISION':
                message.channel.sendTyping();
                message.channel.send(
                    await gpt4VisionController.runPrompt(
                        data.content,
                        data.attachments
                    )
                );
